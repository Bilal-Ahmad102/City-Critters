...
# house_id -> { "steam_id": int, "name": String, "public": bool }
...
# ── Buying ───────────────────────────────────────────────────────────────────
# Called locally when a player interacts with a For-Sale sign.
func request_buy(house_id: String, price: int) -> void:
	...
	if _is_authority():
		_apply_owner(house_id, sid, pname, false)
		_set_owner.rpc(house_id, sid, pname, false)
		_save()
		_notify("You bought %s!" % house_id)
	else:
		# Optimistic: spend now, ask the host to confirm. Refund on denial.
		_pending[house_id] = price
		_srv_buy.rpc_id(1, house_id, sid, pname)
...
func toggle_access(house_id: String) -> void:
	...
	if _is_authority():
		...
		_apply_owner(house_id, int(o["steam_id"]), str(o["name"]), new_public)
		_set_owner.rpc(house_id, int(o["steam_id"]), str(o["name"]), new_public)
		_save()
	else:
...
func _srv_buy(house_id: String, sid: int, pname: String) -> void:
...
func _set_owner(house_id: String, sid: int, pname: String, public: bool) -> void:
	_pending.erase(house_id)
	_apply_owner(house_id, sid, pname, public)
...
func _on_peer_connected(id: int) -> void:
	...
	for hid: String in _owners:
		...
		_set_owner.rpc_id(id, hid, int(o["steam_id"]), str(o["name"]), bool(o["public"]))
...
func _apply_owner(house_id: String, sid: int, pname: String, public: bool) -> void:
	_owners[house_id] = {"steam_id": sid, "name": pname, "public": public}
